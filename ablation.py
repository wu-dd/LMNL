...
# Train the Model
def train_ours5(epoch, train_loader, model1, optimizer1, args):
    train_total = 0
    train_correct = 0
    ...
    for i, (images, images_s1, images_s2, images_s3, labels, indexes) in enumerate(train_loader):
        ...
        logits_s2 = model1(images_s2)
        logits_s3 = model1(images_s3)

        outputs = torch.pow(torch.softmax(logits1, dim=-1).detach(), 1 / (1 + 3)) \
                  * torch.pow(torch.softmax(logits_s1, dim=-1).detach(), 1 / (1 + 3)) \
                  * torch.pow(torch.softmax(logits_s2, dim=-1).detach(), 1 / (1 + 3)) \
                  * torch.pow(torch.softmax(logits_s3, dim=-1).detach(), 1 / (1 + 3))  # outputs = torch.softmax(logits, dim=-1).detach() #outputs = torch.softmax((logits + logits_s1 + logits_s2) / 3, dim=-1).detach()
...
# Evaluate the Model
def evaluate_ensemble(test_loader, model1):
...
################################
os.environ["WANDB_API_KEY"] = "1038e371f2c76d1a383f9c4757c3de55f142fa93"
os.environ["WANDB_MODE"] = "dryrun"
wandb.login()
wandb.init(project="CT", entity="wu-dongdong", config=args, name=args.method)
...
for epoch in range(args.n_epoch):
    ...
    ######################################################################################################################################################################## New Code Starting
    ##########################################################################################################################################################################################
    train_acc1 = train_ours5(epoch, train_loader, model1, optimizer1, args)

    # evaluate models
    test_acc1 = evaluate(test_loader=test_loader, model=model1)
    print('test acc on test images is ', test_acc1)

    # evaluate all models
    test_acc_ensemble = evaluate_ensemble(test_loader, model1)
    ...
    wandb.log({"test_acc": test_acc_ensemble, "epoch": epoch})
    wandb.run.summary["best_acc"] = best_acc_
